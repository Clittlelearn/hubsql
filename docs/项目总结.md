...
├── docs/
│   ├── 架构设计方案.md
│   ├── API文档.md              # 前端对接：全部 REST API 使用与数据格式示例
│   ├── MySQL部署文档.md
│   └── 项目总结.md            # 本文档
...
│   │       ├── lock_record.*         # 锁定记录（LOCK=9）
│   │       ├── unlock_record.*       # 解锁定记录（UNLOCK=10）
│   │       ├── tx_record.*           # 交易记录（滚动，仅最新20高度）
│   │       ├── contract_record.*     # 合约交易（DEPLOY/CALL，含跃入跃出）
│   │       └── claim_record.*        # 申领记录（BONUS=99）
...
│   │   ├── lock_repo.*          # 锁定存储（插入+解锁定标记+查询）
│   │   ├── tx_record_repo.*     # 交易记录存储（滚动清理+查询）
│   │   ├── contract_repo.*      # 合约交易存储（插入+查询）
│   │   └── claim_repo.*         # 申领存储（插入+查询）
